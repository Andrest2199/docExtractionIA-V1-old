...
"""
#%% Define functions
def ocr_openai_vision(file_path, output_folder, data_inject_folder):
    ...
    user_file = utils.read_file(file_path)
    user_prompt.append({"type": "text", 
                    "text": user_file})
    
    # Numero de tokens usados
    # num_tokens = utils.num_tokens_from_messages(user_prompt,"gpt-3.5-turbo-0125")
    # print (f"Tokens in User Prompt: {num_tokens}")
    # num_tokens = utils.num_tokens_from_messages(system_prompt,"gpt-3.5-turbo-0125")
    ...
    json_file_name = file_path.split("/")[-1].replace(f".{file_extension}", ".json")
...
test_file = os.path.join(text_extracted_folder, "14_test.txt")
